layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    form.on('submit(submit)', function () {
        let location = "top:" + $("#frameTop").val() + "rpx;left:" + $("#frameLeft").val() + "rpx";
        let imglocation = "top:" + $("#pointTop").val() + "rpx;left:" + $("#pointLeft").val() + "rpx";
        $("#location").val(location);
        $("#imglocation").val(imglocation);
        $.ajax({
...
function add_image(index) {
    ...
    var c = '<img id="image_' + (index + 1) + '" onclick="click_image(' + (index + 1) + ')" style="cursor:pointer;" src="/images/upload_hover.jpg" width="300px" height="200px"/>'
    var d = '</div>';
